...
importScripts("date-controls.js");
...
if (jaaBrowser.runtime.onMessage && jaaBrowser.scripting) {
  jaaBrowser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (!message || (message.type !== "JAA_MAIN_REPLACE_TEXT" && message.type !== "JAA_MAIN_SET_DATE_SECTION")) return false;
    ...
    function executeMainEdit() {
      if (message.type === "JAA_MAIN_SET_DATE_SECTION") {
        jaaBrowser.scripting.executeScript({ target: target, world: "MAIN", func: jaaSetDateSectionInPage,
          args: [String(message.token), String(message.value == null ? "" : message.value)]
        }).then(function (results) {
          sendResponse(results && results[0] ? results[0].result : { ok: false, error: "Date editing returned no result" });
        }).catch(function (error) { sendResponse({ ok: false, error: String(error.message || error) }); });
        return true;
      }
    ...
    if (message.agent === true) {
      jaaRequirePageActions().then(executeMainEdit).catch(function (error) {
        sendResponse({ ok: false, error: String(error.message || error) });
      });
    } else executeMainEdit();
    return true;
  });
}
